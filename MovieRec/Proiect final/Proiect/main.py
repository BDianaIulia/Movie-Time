...
def top_movies(genre, min_persentage = 0.60):
    ...
    qualified = current_dataset[(current_dataset['vote_count'] >= min_votes) & (current_dataset['vote_count'].notnull())][['title', 'year', 'vote_count', 'vote_average', 'popularity']]
...
qualified = movie_dataset[(movie_dataset['vote_count'] >= min_votes) & (movie_dataset['vote_count'].notnull())][['title', 'year', 'vote_count', 'vote_average', 'popularity', 'genres']]
qualified['vote_count'] = qualified['vote_count'].astype('int')
qualified['vote_average'] = qualified['vote_average'].astype('int')
#print(qualified)
...
# Merge the keword and credits dataset to md dataset based on its ID
...
def recommender(movie_name):
    ...
    movies_details = smd.iloc[movies_idx][['title', 'vote_average', 'vote_count', 'year']]
...
# Compute a similarity matrix, which it's naccessary later, for query
...
def resultForQuery(query):
    ...
    movies_details = gathered_md.iloc[movies_idx][['title', 'year']]
