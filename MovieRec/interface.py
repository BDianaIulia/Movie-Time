...
while True:
    ...
    if values['eTitle2']:
        try:
            window['-OUTPUT-TITLE-2-'].update(main.get_recommendations_overview(values['eTitle2']).head(10))
            print(main.get_recommendations_overview(values['eTitle2']).head(10))
        except ValueError:
            window['-OUTPUT-TITLE-2-'].update('Whoops! Something went wrong.')

    if values['eTitle3']:
        try:
            window['-OUTPUT-TITLE-3-'].update(main.get_recommendations_details(values['eTitle3']))
            print(main.get_recommendations_details(values['eTitle3']))
        except ValueError:
            window['-OUTPUT-TITLE-3-'].update('Whoops! Something went wrong.')

    if values['eKeyWord']:
        try:
            window['-OUTPUT-KEY-WORD-'].update(main.get_recommendations_keywords(values['eKeyWord']))
            print(main.get_recommendations_keywords(values['eKeyWord']))
        except ValueError:
